...
function checkAutoExitBySensor(spotName, currentStatus) {
    ...
    if (activeSpot === spotName && currentStatus === 1) {
        ...
        const finalDuration = sessionStorage.getItem('finalDuration'); // เวลาที่จองไว้ (เช่น 01:00:00)
        ...
        // 2. Logic เช็กการจอดเกินเวลา (Overtime Calculation)
        // แปลงเวลาที่จองไว้เป็นมิลลิวินาที
        const pTime = finalDuration.split(':');
        const bookedMs = ((+pTime[0] * 3600) + (+pTime[1] * 60) + (+pTime[2])) * 1000;
        
        let overtimeMinutes = 0;
        if (diffMs > bookedMs) {
            overtimeMinutes = Math.floor((diffMs - bookedMs) / (1000 * 60)); // ส่วนต่างที่เกินมาเป็นนาที
        }
