...
// =========================================================
const RESERVED_SPOTS = ["P01", "P02"]; // ใส่ชื่อช่องที่มีเจ้าของประจำ
        ...
        parkingLayer = L.geoJson(data, {
            ...
            onEachFeature: function(feature, layer) {
                if (feature.properties.name) {
                    ...
                    if (RESERVED_SPOTS.includes(spotName)) {
                        layer.bindPopup(`🚨 <b>ที่จอดรถส่วนบุคคล (${spotName})</b><br>ช่องจอดนี้มีเจ้าของประจำ`);
                    }

                    // 2. ช่องอื่นๆ (รวมทั้ง Active และช่องทั่วไป)
                    else {
...
function processCheckIn(spotName, exitTimeStr) {
    // 1. ส่งเวลาขึ้น Blynk
    const vPinTime = TIME_MAPPING[spotName]; 
    const blynkUpdateUrl = `${BLYNK_BASE_URL}update?token=${BLYNK_AUTH_TOKEN}&${vPinTime}=${exitTimeStr}`;
    
    fetch(blynkUpdateUrl)
        .then(() => console.log(`DEBUG: Time synced to Blynk`))
    ...
    if (exitTime <= now) {
        Swal.fire('ผิดพลาด', 'กรุณาระบุเวลาที่มากกว่าเวลาปัจจุบัน!', 'error');
        hasArrivedPrompted = false;
        return;
    }

    // 2. อัปเดต UI หน้าจอตัวเอง
    document.getElementById('route-display').innerHTML = `
...
function startCountdown(endTime, spotName) {
    ...
    function updateTimer() {
        ...
        if (distance <= 0) {
            ...
            timeString = "กำลังจะว่าง";
            
            // อัปเดตบนแผนที่ให้เป็นข้อความเตือน
            updateTooltipOnMap(spotName, timeString);
            
            // อัปเดตในกล่องขาว
            const timerElem = document.getElementById('countdown-timer');
            if (timerElem) timerElem.innerHTML = "หมดเวลาจอด";
            
            alert(`ช่องจอด ${spotName} หมดเวลาจอดแล้ว!`);
            return;
        }
