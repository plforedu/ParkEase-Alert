...
function processCheckIn(spotName, exitTimeStr) {
    // 1. ต้องประกาศตัวแปรเวลา "ตอนนี้" ให้ชัดเจนทุกครั้งที่เรียกฟังก์ชัน
    const now = new Date(); 
    
    const [hours, minutes] = exitTimeStr.split(':');
    // สร้าง Object เวลาขาออกของวันนี้
    const exitTime = new Date(now.getFullYear(), now.getMonth(), now.getDate(), hours, minutes, 0);

    // 2. เช็คกันเหนียว: ถ้าเวลาออกน้อยกว่าเวลาเข้า (เช่นข้ามวัน) ให้แจ้งเตือน
    if (exitTime <= now) {
        Swal.fire('ผิดพลาด', 'เวลาออกต้องมากกว่าเวลาปัจจุบัน!', 'error');
        return;
    }

    // 3. หัวใจสำคัญ: คำนวณส่วนต่างใหม่ทุกรอบ (Duration = Exit - Now)
    const diffMs = exitTime.getTime() - now.getTime(); 
    
    // แปลงมิลลิวินาทีเป็น ชั่วโมง:นาที:วินาที
    const diffHrs = Math.floor(diffMs / (1000 * 60 * 60));
    const diffMins = Math.floor((diffMs % (1000 * 60 * 60)) / (1000 * 60));
    const diffSecs = Math.floor((diffMs % (1000 * 60)) / 1000);

    // จัด Format ใหม่ให้ชัวร์
    const totalDuration = `${String(diffHrs).padStart(2, '0')}:${String(diffMins).padStart(2, '0')}:${String(diffSecs).padStart(2, '0')}`;
    
    // 4. ล้างค่าเก่าก่อนบันทึกค่าใหม่ (ป้องกัน Browser จำค่าเดิม)
    sessionStorage.removeItem('finalDuration');
    sessionStorage.setItem('finalDuration', totalDuration);
    sessionStorage.setItem('finalSpot', spotName);
    // ------------------------------------------------

    // 2. อัปเดต UI หน้าจอตัวเอง (โค้ดเดิม)
    document.getElementById('route-display').innerHTML = `
    ...
    lastSelectedSpot = null; 
    currentParkingStatus[spotName] = 0; 
    updateParkingStyle(currentParkingStatus); 
}

function startCountdown(endTime, spotName) {
    // 1. เคลียร์ Interval เก่าหากมีการรันค้างอยู่ (กันตัวเลขเด้งไปมา)
    ...
    function updateTimer() {
        ...
        if (distance <= 0) {
            ...
            const duration = sessionStorage.getItem('finalDuration'); 
            const spot = sessionStorage.getItem('finalSpot');

            // หาพิกัดของช่องจอดที่เลือกไว้ เพื่อส่งไปหน้า index
            let lat = 13.6515, lng = 100.4965; // ค่า Default
            parkingLayer.eachLayer(function(layer) {
                if (layer.feature.properties.name === spot) {
            ...
            Swal.fire({
                title: 'หมดเวลาจอดแล้ว!',
                text: 'กำลังพาคุณไปหน้าชำระเงิน...',
                icon: 'success',
                timer: 3000,
                showConfirmButton: false
            }).then(() => {
                // เพิ่ม &lat=${lat}&lng=${lng} เข้าไปใน URL
                window.location.href = `index.html?status=pay&duration=${duration}&spot=${spot}&lat=${lat}&lng=${lng}`;
            });
            return;
        }
        ...
        // สร้าง Format HH:MM:SS
        timeString = `${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;

        // 4. สั่งอัปเดตตัวเลขลงบนแผนที่ (เรียกฟังก์ชันใหม่ที่เรากำลังจะเพิ่ม)
        updateTooltipOnMap(spotName, timeString);

        // 5. อัปเดตตัวเลขในกล่องขาวด้านขวา (UI เดิม)
        const timerElem = document.getElementById('countdown-timer');
        if (timerElem) timerElem.innerHTML = timeString;
    }

    // เรียกทำงานทันที 1 ครั้ง และตั้งให้ทำงานทุกวินาที
    updateTimer(); 
...
function renderGlobalCountdown(spotName, exitTimeStr) {
    ...
    // หาตำแหน่งช่องจอดและแปะป้าย Tooltip
    parkingLayer.eachLayer(function(layer) {
        if (layer.feature.properties.name === spotName) {
            const center = layer.getBounds().getCenter();
            if (!spotTooltips[spotName]) {
                spotTooltips[spotName] = L.tooltip({ permanent: true, direction: 'center', className: 'countdown-tooltip' });
            }
            spotTooltips[spotName].setLatLng(center).setContent(`<b style="color:red;">${timeText}</b>`).addTo(map);
        }
    });
}